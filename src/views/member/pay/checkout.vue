...
export default {
  ...
  setup () {
    ...
    // 提交订单需要收货地址（checkout-address传递出来的）
    ...
    const submit = () => {
      ...
      submitOrder(reqParams).then(data => {
        Message({ type: 'success', text: '已提交订单' })
        // 跳转到支付页面，并携带id
        router.push(`/member/pay?orderId=${data.result.id}`)
      })
    }

    return { order, changeAddress, reqParams, submit }
  }
}
</script>
